...
public class UseCashItemHandler extends AbstractMaplePacketHandler {
    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (player.getInventory(MapleInventoryType.CASH).getItem(slot).getItemId() != itemId) {
            c.disconnect();
            c.getSession().close();
            return;
        }
        if (!c.getPlayer().haveItem(itemId, 1, true, true)) {
            c.disconnect();
            c.getSession().close();
            return;
        }
            ...
            try {
                switch (itemType) {
                    ...
                    case 506: // hmm npe when double clicking..
                        ...
                        slea.readInt(); // some time stamp shit.
                    ...
                    case 524:
                        for (int i = 0; i < 3; i++) {
                            ...
                            if (pet != null || player.getInventory(MapleInventoryType.CASH).getItem(slot) != null) {
                                if ((itemId == 5240012 && pet.getItemId() >= 5000028 && pet.getItemId() <= 5000033) || (itemId == 5240021 && pet.getItemId() >= 5000047 && pet.getItemId() <= 500005) ||
                                        (itemId == 5240006 && (pet.getItemId() <= 5000007 || pet.getItemId() <= 5000010 || pet.getItemId() <= 5000012))) {
                                    pet.setFullness(100);
                    ...
                    case 504:
                        ...
                        break;
                    default:
                        log.warn("Non-existant cash item was used   itemid: {}", itemId);
                }
                c.getSession().write(MaplePacketCreator.enableActions());
