...
public class WorldRegistryImpl extends UnicastRemoteObject implements WorldRegistry {
    ...
    private WorldRegistryImpl(boolean initTerritories) throws RemoteException {
        ...
        if (initTerritories) {
            territoryStorage = new TerritoryStorage();
            territoryStorage.loadTerritoriesFromDb();
            territoryStorage.loadMonstersFromDb();
        }

    }

    public static WorldRegistryImpl getInstance(boolean initTerritories) {
        if (instance == null) {
            try {
                instance = new WorldRegistryImpl(initTerritories);
    ...
    public void setAllianceNotice(int id, String anotice) {
        if (alliances.containsKey(id)) {
            getAlliance(id).setNotice(anotice);
        }
    }
    ...
    public boolean setGuildAlliance(int guildid, MapleAlliance alliance) {
        MapleGuild guild = guilds.get(guildid);
        if (guild != null) {
            guild.setAlliance(alliance); // any monster? yea i killed one and it says hm is it a int in database? yeah  then just do this o0 i have 0 points i  killed a snail ah fuck hold on
            return true;
        }
        return false;
    }
    ...
    public void resetTerritoryPoints(int id) throws RemoteException {
        MapleGuild[] guildsArray = guilds.values().toArray(new MapleGuild[guilds.size()]);

        for (int i = 0; i < guildsArray.length; i++) {
            guilds.get(guildsArray[i].getId()).setTerritoryPoints(id, 0);
        }
    }

    public void updateTerritory(Territory t, int id) throws RemoteException {
        territoryStorage.updateTerritory(t, id);
    }
    ...
    public void broadcastPacketToWorld(MaplePacket packet) throws RemoteException {
        ChannelWorldInterface[] csArray = channelServer.values().toArray(new ChannelWorldInterface[channelServer.size()]);

        for (int i = 0; i < csArray.length; i++) {
            channelServer.get(csArray[i].getChannelId()).broadcastPacket(packet);
        }
    }

    public void addPointsToTerritory(int guildid, int tid, int points) {
        MapleGuild g = guilds.get(guildid);
        if (g != null) {
            g.addPointsToTerritory(tid, points);
        }
    }
}
