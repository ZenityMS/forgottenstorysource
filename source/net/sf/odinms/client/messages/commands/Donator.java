...
public class Donator implements Command {
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception {
        ...
        if (splitted[0].equals("#buffme")) {
        ...
        } else if (splitted[0].equals("#goto") || splitted[0].equals("!goto")) {
        ...
        } else if (splitted[0].equalsIgnoreCase("#dsay")) {
            if (splitted.length > 1) {
                try {
                    cserv.getWorldInterface().broadcastMessage(player.getName(), MaplePacketCreator.serverNotice(5, "Donator: [" + player.getName() + "] " + StringUtil.joinStringFrom(splitted, 1)).getBytes());
        ...
        } else if (splitted[0].equalsIgnoreCase("#dzakum")) {
            if (!player.getCheatTracker().Spam(600000, 0)) {
                if (player.getMapId() != 910000000 && player.getMapId() != 100000000) {
                    ...
                    player.dropMessage("You can't spawn zakum in henesys or the free market.");
                }
            } else {
                player.dropMessage("You can only spawn zakum every 10 minutes.");
            }
        } else if (splitted[0].equalsIgnoreCase("#kilmonsters")) {
            String mapMessage = "";
            ...
            if (player.getDonatorLevel() > 0) {
                player.changeMap(912000000, 0);
                player.dropMessage("You are now being warped to the donator map...");
            } else {
                player.dropMessage("You are not a donator.");
            }
        } else if (splitted[0].equals("#dstorage")) {
    ...
    public CommandDefinition[] getDefinition() {
        return new CommandDefinition[]{
                    ...
                    new CommandDefinition("dstorage", 1),
                    new CommandDefinition("dsay", 1),
                    new CommandDefinition("dzakum", 1),
                    new CommandDefinition("dmap", 1),
                    new CommandDefinition("killmonsters", 1)
                };
    }
}
