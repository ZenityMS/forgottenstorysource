...
function playerEntry(eim, player) {
	...
	player.getClient().getSession().write(net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "Omega Sector is under attack. Protect it!!"));
	player.getClient().getSession().write(net.sf.odinms.tools.MaplePacketCreator.musicChange("Bgm14/DragonNest"));
	//THE CLOCK IS SHIT
		...
		if (eim.getPlayerCount() > 0) {
			if (map.countMonster(eim.getPlayers().get(0)) < 30) {
				for (var i = 0; i < times && map.countMonster(eim.getPlayers().get(0)) < 30; i++) {
						var mob = net.sf.odinms.server.life.MapleLifeFactory.getMonster(mobSpawn);
