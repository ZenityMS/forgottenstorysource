...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (cm.getPlayer().getEventInstance() == null) {
			if (status == 0) {
				cm.sendNext("Recently, there has been an attack on #bOmega Sector#k. All the bosses from the MapleKarma world have entered this dimension, become much stronger, have more HP and are invading #bOmega Sector#k. We need a brave team of people to stop them.");
			} else if (status == 1) {
				...
				if (!cm.isLeader()) { // not party leader
					...
					if (next) {
						...
						var em = cm.getEventManager("BossHunterPQ");
						if (em == null) {
