        ...
        if (status == 0) {
           ...
           if (eim != null) {           
            if (chr.isLeader()) {
                    ...
                    chr.removeAll(1302065); // Blue Flag
                    chr.removeAll(1302033); // Red Flag
                    chr.flagPromptName2 = cm.getPlayer().getName();
                    chr.gainItem(1302065, 1, false, false);
                    cm.getPlayer().setRedTeam();
                    cm.getPlayer().setCanPickup(false);
                    cm.getPlayer().hasflag = true;
                    // cm.getPlayer().removeAll(4001025);
                    cm.getPlayer().removeAll(1302065); // Blue Flag
                    cm.getPlayer().removeAll(1302033); // Red Flag
                    cm.getPlayer().flagPromptName2 = cm.getPlayer().getName();
                    cm.getPlayer().gainItem(1302033, 1, false, false);
