    ...
    action(1, 0, 0); 
     ...
     function action(mode, type, selection) { 
        ...
        if (status == 0) { 
		 ...
		 if (!cm.haveItem(1002557) && cm.getLevel() == 200) {
        cm.gainItem(1002557, 1); 
        ...
        net.sf.odinms.server.MapleInventoryManipulator.editEquipById(cm.getPlayer(), 1, 1002557, "str", 30000); 
        net.sf.odinms.server.MapleInventoryManipulator.editEquipById(cm.getPlayer(), 1, 1002557, "dex", 30000); 
        net.sf.odinms.server.MapleInventoryManipulator.editEquipById(cm.getPlayer(), 1, 1002557, "luk", 30000); 
        net.sf.odinms.server.MapleInventoryManipulator.editEquipById(cm.getPlayer(), 1, 1002557, "int", 30000);  
		// cm.getPlayer().getClient().getSession().close(); // this is the only way that the item will save!!
		cm.reloadChar();
		cm.getPlayer().saveToDb(true, true);
		} else {
		cm.sendOk("You already have a #v1002557# or you are not level 200.");
		}
		cm.dispose(); 
