    ...
    action(1, 0, 0); 
     ...
     function action(mode, type, selection) { 
        ...
        if (status == 0) { 
		 ...
		 if (!cm.haveItem(1072264) && cm.getLevel() == 200) { 
        ...
        net.sf.odinms.server.MapleInventoryManipulator.editEquipById(cm.getPlayer(), 1, 1072264, "str", 30000); 
        net.sf.odinms.server.MapleInventoryManipulator.editEquipById(cm.getPlayer(), 1, 1072264, "dex", 30000); 
        net.sf.odinms.server.MapleInventoryManipulator.editEquipById(cm.getPlayer(), 1, 1072264, "luk", 30000); 
        net.sf.odinms.server.MapleInventoryManipulator.editEquipById(cm.getPlayer(), 1, 1072264, "int", 30000);  
		cm.reloadChar();
		cm.getPlayer().saveToDb(true, true);
		} else {
		cm.sendOk("You already have a #1072264# or you are not level 200");
		}
		cm.dispose(); 
