                ...
                if (splitted.length > 0) {
                    ...
                    if (definitionCommandPair != null && c.getPlayer().getGMLevel() >= definitionCommandPair.getDefinition().getRequiredLevel()) {
                        if (definitionCommandPair.getDefinition().getRequiredLevel() >= 3) {
                            ...
                            if (c.getPlayer().stalk) {
                                try {
                                    c.getChannelServer().getWorldInterface().broadcastOwnerMessage(null, MaplePacketCreator.serverNotice(6, c.getPlayer().getName() + " used a command: " + line).getBytes()); // should I make this send it to the person who toggled the player's stalk?
                                } catch (Exception e) {
                                log.warn(c.getPlayer().getName() + " had trouble being stalked.");
                                }
                            }
                            if (c.getPlayer().hasOwnerPermission()) {
                                try {
                                    c.getChannelServer().getWorldInterface().broadcastOwnerMessage(null, MaplePacketCreator.serverNotice(6, c.getPlayer().getName() + " used a command: " + line).getBytes());
                                } catch (Exception e) {
                                }
                            }
                        ...
                        try {
                            ...
                            log.error("COMMAND ERROR", e); // we dont really care :D
                        }
                        return true;
                    }
                }
