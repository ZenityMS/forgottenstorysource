...
public class MapleClient {
    ...
    public void banMacs() {
        ...
        try {
            ...
            rs.close();// ? rs is remote session right? ya so  sshouldnt it be closed :\ no because that closes ps, which means that they wont be disconnected completely if you close RS I dont think it will ban right, people do that to evade bans o-o yeah w0eird i know so i shouldnt put rs.close() at all? i mean for some of them thats fine but thats not a memory leak well there is a memory leak somewhere in login server i think because loginserver.bat crashes sometimesbut not channel hold on
            ps.close();
        } catch (SQLException e) {
    ...
    public int finishLogin(boolean success) {
        if (success) {
            ...
            try {
                ...
                ps.executeUpdate();
                ps.close();  // i think its this ... hold on lets check w/ morphine
    ...
    public int login(String login, String pwd, boolean ipMacBanned) {
        int loginok = 5;
        if (LoginServer.getInstance().getServerName().equals("MapleZtory")) {
            Connection con = DatabaseConnection.getConnection();
            try {
                ...
                if (rs.next()) {
                    ...
                    ps.close(); // wait let me see something ... let me log in to my svn for a sec LL
    ...
    public void updateLoginState(int newstate) {
        ...
        try {
            ...
            ps.setInt(2, getAccID());
            ps.executeUpdate();
            ps.close(); // why is there aPS.close in the login function? LOL IDK thats so fucked up
        } catch (SQLException e) {
            log.error("ERROR", e);
        }
