...
public class PvPLibrary {
    ...
    public static void doFamilyPvP(MapleCharacter player, MapleMap map, AbstractDealDamageHandler.AttackInfo attack) {
        ...
        for (MapleCharacter attackedPlayers : player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters())) {
            if (attackedPlayers.isAlive() && (player.getFamily() != attackedPlayers.getFamily())) {
    ...
    public static void doPvP(MapleCharacter player, MapleMap map, AbstractDealDamageHandler.AttackInfo attack) {
        ...
        for (MapleCharacter attackedPlayers : player.getMap().getNearestPvpChar(player.getPosition(), maxDis, maxHeight, player.getMap().getCharacters())) {
            if (attackedPlayers.isAlive() && (player.getParty() == null || player.getParty() != attackedPlayers.getParty())) {
                if (attackedPlayers.timeshit >= 2 && attackedPlayers.blueteam) {
                    attackedPlayers.changeMap(1010000, 0);
                    attackedPlayers.dropMessage("You have been warped back because you have gotten two hits");
                    attackedPlayers.timeshit = 0;
                } else {
                    if (attackedPlayers.timeshit >= 2 && attackedPlayers.redteam) {
                ...
                if (flagmap == attackedPlayers.getMapId()) {
                    if (!attackedPlayers.hasflag) {
                        attackedPlayers.timeshit = attackedPlayers.timeshit + 1;
                    } else {
                        IItem blue = new Item(1302065, (byte) 3, (short) 1);
                        IItem red = new Item(1302033, (byte) 3, (short) 1);
                        if (attackedPlayers.blueteam) {
                            attackedPlayers.removeAll(1302065);
                            attackedPlayers.getMap().spawnItemDrop(player, player, blue, attackedPlayers.getPosition(), true, false);
                        } else {
                            if (attackedPlayers.redteam) {
                                attackedPlayers.removeAll(1302033);
                                attackedPlayers.getMap().spawnItemDrop(player, player, red, attackedPlayers.getPosition(), true, false);
                            } else {
                                // put something in the log?
                            }
                        }
                        attackedPlayers.hasflag = false;
                        attackedPlayers.setCanPickup(true);
                    }
                }
                monsterBomb(player, attackedPlayers, map, attack);
            }
