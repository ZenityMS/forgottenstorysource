        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (createType == 3) { // trade
                ...
                if (createType == 1 || createType == 2) {
                    ...
                    c.getPlayer().dropMessage("Remember that if there is a server shutdown, your items WILL be lost! Please make a hired merchant if possible!");
                    }
                    if (c.getPlayer().hasMerchant()) {
                        c.getPlayer().dropMessage(1, "Close your hired merchant, please!");
                        return;
                    }
                } else if (createType == 5) {
                    if (c.getPlayer().hasMerchant()) {
                        c.getPlayer().dropMessage("Please kill your hired merchant!!");
                        return;
                    }
            ...
            if (c.getPlayer().getTrade() != null && item != null) {
                if (checkq > 4000) {
                    AutobanManager.getInstance().autoban(c, "PE while in trade.");
                }
            ...
            if (shop != null && shop.isOwner(c.getPlayer())) {
                if (ivItem != null && ivItem.getQuantity() >= bundles * perBundle) {
                    if (price < 0) {
                        AutobanManager.getInstance().autoban(c, "PE adding a negative priced item.");
                        return;
                    }
                    if (bundles <= 0 || perBundle <= 0 || checkquantity > 20000 || checksmquantity < 0 || checkiquantity < 0 || checkiquantity > 20000) {
                        AutobanManager.getInstance().autoban(c, "PE Player Shop item: " + sellItem.getItemId());
                        return;
                    }
