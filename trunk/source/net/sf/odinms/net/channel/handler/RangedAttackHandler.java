    ...
    public void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (attack.skill == 5121002) {
            ...
            if (projectile != 0 || soulArrow) {
                ...
                // This should fix stuck attacks with bows if I'm right.
                if (c.getPlayer().hasFakeChar()) {
                    for (FakeCharacter ch : c.getPlayer().getFakeChars()) {
                        player.getMap().broadcastMessage(ch.getFakeChar(), MaplePacketCreator.rangedAttack(ch.getFakeChar().getId(), attack.skill, attack.direction, attack.numAttackedAndDamage, visProjectile, attack.allDamage, attack.speed), false, true);
                        applyAttack(attack, ch.getFakeChar(), maxdamage, bulletCount);
                    }
                }
            }
        }
